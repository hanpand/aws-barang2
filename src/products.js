import product1 from './assets/1.JPG';
import product2 from './assets/2.JPG';
import product3 from './assets/3.JPG';
import product4 from './assets/4.JPG';
import product5 from './assets/5.JPG';
import product6 from './assets/6.JPG';
import product7 from './assets/7.JPG';
import product8 from './assets/8.JPG';
import product9 from './assets/9.JPG';
import product10 from './assets/10.JPG';
import product11 from './assets/11.JPG';
import product12 from './assets/12.JPG';
import product13 from './assets/13.JPG';
import product14 from './assets/14.JPG';
import product15 from './assets/15.JPG';
import product16 from './assets/16.JPG';




export const PRODUCTS = [
    {
        id: 1,
        productName: "Heizlüfter",
        price: 0,
        productImage: product1
    },
    {
        id: 2,
        productName: "Ultraboost x Nice Kicks (Not Available)",
        price: 0,
        productImage: product2
    },
    {
        id: 3,
        productName: "Reuter TP-Link",
        price: 0,
        productImage: product3
    },
    {
        id: 4,
        productName: "Lampu Belajar",
        price: 0,
        productImage: product4
    },
    {
        id: 5,
        productName: "Tempat Makan Ikea (3 pcs)",
        price: 0,
        productImage: product5
    },
    {
        id: 6,
        productName: "Talenan (Biru)",
        price: 0,
        productImage: product6
    },
    {
        id: 7,
        productName: "Talenan (Abu) ",
        price: 0,
        productImage: product7
    },
    {
        id: 8,
        productName: "Name?",
        price: 0,
        productImage: product8
    },
    {
        id: 9,
        productName: "Centong",
        price: 0,
        productImage: product9
    },
    {
        id: 10,
        productName: "Pisau",
        price: 0,
        productImage: product10
    },
    {
        id: 11,
        productName: "Keset",
        price: 0,
        productImage: product11
    },
    {
        id: 12,
        productName: "Sapu Kecil",
        price: 0,
        productImage: product12
    },
    {
        id: 13,
        productName: "Alat buat Plakban",
        price: 0,
        productImage: product13
    },
    {
        id: 14,
        productName: "Stander HP (Wireless Charging)",
        price: 0,
        productImage: product14
    },
    {
        id: 15,
        productName: "Rice Cooker",
        price: 0,
        productImage: product15
    },
    {
        id: 16,
        productName: "Water Cooker",
        price: 0,
        productImage: product16
    }


]